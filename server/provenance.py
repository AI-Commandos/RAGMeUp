...
def compute_llm_provenance(tokenizer, model, query, context, answer):
    ...
    for doc in context:
        ...
        context_thread = [
            {'role': 'user', 'content': prompt.format_map({"query": query, "context": new_doc, "answer": answer})}]
...
class DocumentSimilarityAttribution:
    ...
    def compute_similarity(self, query, context, answer):
        ...
        normalized_scores = [score / total_similarity for score in
                             similarity_scores] if total_similarity > 0 else similarity_scores
