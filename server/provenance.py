...
def compute_attention(model, tokenizer, thread, query, context, answer):
    ...
    thread_tokens = tokenizer.encode(
        thread, return_tensors="pt", add_special_tokens=False
    )

    # Compute the attention
    with torch.no_grad():
        output = model(
            input_ids=thread_tokens, output_attentions=True, attn_implementation="eager"
        )
    ...
    query_start, query_end = find_sublist_positions(
        thread_tokens[0].tolist(), query_tokens
    )
    answer_start, answer_end = find_sublist_positions(
        thread_tokens[0].tolist(), answer_tokens
    )
    ...
    for part in context:
        ...
        context_offsets.append(
            find_sublist_positions(thread_tokens[0].tolist(), part_tokens)
        )
    ...
    query_to_query = (
        attentions[0, :, query_start:query_end, query_start:query_end].mean().item()
    )
    if query_to_query > 0:
        total_attention.append(query_to_query)
    query_to_answer = (
        attentions[0, :, query_start:query_end, answer_start:answer_end].mean().item()
    )
    if query_to_answer > 0:
        total_attention.append(query_to_answer)
    answer_to_answer = (
        attentions[0, :, answer_start:answer_end, answer_start:answer_end].mean().item()
    )
    if answer_to_answer > 0:
        total_attention.append(answer_to_answer)
    answer_to_query = (
        attentions[0, :, answer_start:answer_end, query_start:query_end].mean().item()
    )
    ...
    for start, end in context_offsets:
        ...
        doc_attention = []
        answer_to_doc = (
            attentions[0, :, answer_start:answer_end, start:end].mean().item()
        )
        if answer_to_doc > 0:
            doc_attention.append(answer_to_doc)
        doc_to_answer = (
            attentions[0, :, start:end, answer_start:answer_end].mean().item()
        )
        if doc_to_answer > 0:
        ...
        if include_query:
            ...
            query_to_doc = (
                attentions[0, :, query_start:query_end, start:end].mean().item()
            )
            if query_to_doc > 0:
                doc_attention.append(query_to_doc)
            doc_to_query = (
                attentions[0, :, start:end, query_start:query_end].mean().item()
            )
            if doc_to_query > 0:
...
def compute_llm_provenance(tokenizer, model, query, context, answer):
    ...
    for doc in context:
        ...
        context_thread = [
            {
                ...
                "content": prompt.format_map(
                    {"query": query, "context": new_doc, "answer": answer}
                ),
            }
        ]
        ...
        output_ids = model.generate(
            input_ids=input_ids, attention_mask=attention_mask, max_new_tokens=10
        )
...
class DocumentSimilarityAttribution:
    def __init__(self):
        ...
        self.model = SentenceTransformer(
            os.getenv("provenance_similarity_llm"), device=device
        )
    ...
    def compute_similarity(self, query, context, answer):
        ...
        for i, doc_embedding in enumerate(context_embeddings):
            ...
            doc_answer_similarity = cosine_similarity(
                [doc_embedding], [answer_embedding]
            )[0][0]
        ...
        normalized_scores = (
            [score / total_similarity for score in similarity_scores]
            if total_similarity > 0
            else similarity_scores
        )
