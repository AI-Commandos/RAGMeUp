...
def evaluate_deep_eval():
    ...
    if not isinstance(context, (list, str)):
        return jsonify({"error": "Context must be a string or a list of strings"}), 400
    ...
    try:
        ...
        # Debugging logs
        app.logger.info(f"Documents: {context}")
        app.logger.info(f"Formatted Context: {formatted_context}")

        # Generate response using the RAG pipeline
        try:
            # Use handle_user_interaction for better integration with RAG pipelines
            thread, response = raghelper.handle_user_interaction(
                user_query=question,
                history=[{"role": "user", "content": question}]  # Pass the question as part of the conversation history
            )
            ...
            if not generated_response:
                raise ValueError("No response content generated by the LLM.")
        except Exception as e:
            app.logger.error(f"Error during RAG pipeline response generation: {str(e)}", exc_info=True)
            raise

        # Debugging extracted answer
        ...
        # Debugging test case
        app.logger.info(f"Test Case: {test_case}")

        # Evaluate the test case with metrics
        results = evaluate([test_case], metrics)

        # Debugging raw results
        app.logger.info(f"Raw Evaluation Results: {results}")

        # Serialize evaluation results
        serialized_results = []
        if isinstance(results, list):  # Ensure results is iterable
            for result in results:
                serialized_results.append({
                    "metric": getattr(result, "metric", "Unknown Metric"),
                    "score": getattr(result, "score", 0.0),
                    "reason": getattr(result, "reason", "No reason provided"),
                    "threshold": getattr(result, "threshold", None),
                })
        else:
            app.logger.error(f"Unexpected results format: {type(results)}")

        # Debugging serialized results
        app.logger.info(f"Serialized Results: {serialized_results}")
        ...
        app.logger.error(f"Error during evaluation: {str(e)}", exc_info=True)
